...
def delete_image_healthycheck_info(db_resource, project_id, healthcheck_id):
    ...
    table_healthycheck_info.delete_item(
        Key={"project_id": project_id, "healthcheck_id": healthcheck_id}
    )
    ...
    with table_project.batch_writer() as batch:
        for each in queryResponse["Items"]:
            ...
            resp = table_project.update_item(
                ...
                ExpressionAttributeValues={":r": new_reference_image},
...
def lambda_handler(event, context):
    try:
        ...
        for object in ls_object_info:
            ...
            request = {"filename": object["filename"]}
            dict_request[type_method]["ls_rq"].append(request)
    except Exception as e:
        return generate_response(
            message=str(e), status_code=HTTPStatus.OK, data={}, error=False
        )
    ...
    try:
        ...
        for key, value in dict_request.items():
            ...
            for request in value["ls_rq"]:
                item = table.get_item(
                    Key={"project_id": project_id, "filename": request["filename"]}
                )
                try:
                    ...
                    print(e)
                if ("healthcheck_id" in item["Item"]) and (
                    not item["Item"]["healthcheck_id"] is None
                    or isinstance(item["Item"]["healthcheck_id"], str)
                ):
                    delete_image_healthycheck_info(
                        db_resource=db_resource,
                        project_id=project_id,
                        healthcheck_id=item["Item"]["healthcheck_id"],
                    )
        ...
        if response.get("Item"):
            ...
            if thum_name:
                ...
                response = table_ori.get_item(
                    Key={"project_id": project_id, "filename": thum_name}
                )
                ...
                if response.get("Item", None) is None:
                    ...
                    response = table_sum_all.update_item(
                        ...
                        ExpressionAttributeNames={"#TK": "thu_key", "#TN": "thu_name"},
        ...
        print("Error: ", repr(e))
        return generate_response(
            message=str(e), status_code=HTTPStatus.OK, data={}, error=True
        )
