...
class GenerateImageClass(LambdaBaseClass):
    ...
    def parser(self, body):
        ...
        self.data_type = body.get(
            KEY_NAME_DATA_TYPE, "ORIGINAL"
        )  # type is one of ORIGINAL or PREPROCESS, default is original
        self.num_aug_per_imgs = min(
            MAX_NUMBER_GEN_PER_IMAGES, body.get(KEY_NAME_NUM_AUG_P_IMG, 1)
        )  # default is 1
        self.data_number = body[
            KEY_NAME_DATA_NUMBER
        ]  # array of number data in train/val/test  [100, 19, 1]
        self.process_type = body.get(
            KEY_NAME_PROCESS_TYPE, VALUE_TYPE_METHOD_PREPROCESS
        )
    ...
    def _check_running_task(
        self, generate_task_model: GenerateTaskModel, identity_id, project_id
    ):
        """
        Check any running tasks of this project
        """
        ls_running_task = generate_task_model.query_running_tasks(
    ...
    def _check_generate_times_limitation(self, identity_id, project_name, type_method):
        ...
        times_generated = int(
            project_rec.get_value_w_default(ProjectItem.FIELD_TIMES_AUGMENT, 0)
        )
        times_preprocess = int(
            project_rec.get_value_w_default(ProjectItem.FIELD_TIMES_PREPRO, 0)
        )
        s3_prefix = project_rec.__dict__[ProjectItem.FIELD_S3_PREFIX]

        if type_method == VALUE_TYPE_METHOD_AUGMENT:
            if times_generated >= int(
                self.const.get_param(os.environ["LIMIT_AUGMENT_TIMES"])
            ):
                raise Exception(
                    MESS_REACH_LIMIT_AUGMENT.format(
                        self.const.get_param(os.environ["LIMIT_AUGMENT_TIMES"])
                    )
                )
        elif type_method == VALUE_TYPE_METHOD_PREPROCESS:
            if times_preprocess >= int(
                self.const.get_param(os.environ["LIMIT_PROCESSING_TIMES"])
            ):
                raise Exception(
                    MESS_REACH_LIMIT_PREPROCESS.format(
                        self.const.get_param(os.environ["LIMIT_PROCESSING_TIMES"])
                    )
                )
    ...
    def _update_generate_times(
        ...
        self.project_model.update_project_generate_times(
            identity_id,
            project_name,
            times_augment,
            times_preprocess,
    ...
    def _create_task(self, identity_id, project_id, type_method):
        # create task id
        task_id = self.generate_task_model.create_new_generate_task(
    ...
    def handle(self, event, context):
        ...
        if type_method == VALUE_TYPE_METHOD_PREPROCESS:
            self.ls_methods_id = self._update_ls_method_for_preprocessing(
                self.ls_methods_id
            )
        ...
        (
            ...
            times_preprocess,
            s3_prefix,
        ) = self._check_generate_times_limitation(
        ...
        times_preprocess, times_augment = self._update_generate_times(
            identity_id,
            self.project_name,
            type_method,
            times_augment,
        ...
        if type_method == VALUE_TYPE_METHOD_AUGMENT:
            self.project_model.update_project_info(
                identity_id, self.project_name, self.data_type, self.data_number
            )

        ### check if preprocess then reset in prj sumary
        if type_method == VALUE_TYPE_METHOD_PREPROCESS:
            self.project_sum_model.reset_prj_sum_preprocess(
                project_id=self.project_id, type_data=VALUE_TYPE_DATA_PREPROCESSED
            )
            ...
            print(
                f"The number of files in preproces table {len(queryResponse['Items'])}"
            )
            with preprocessTBL.batch_writer() as batch:
                for each in queryResponse["Items"]:
                    preprocessTBL.delete_item(
                        Key={
