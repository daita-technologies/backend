...
class ListMethodsClass(LambdaBaseClass):
    def __init__(self) -> None:
        super().__init__()
        self.method_model = MethodModel(os.environ["TABLE_METHODS_NAME"])

    def _restructure_items_response(self, items: List[MethodItem]):

        dict_type = {KEY_NAME_RES_AUMENTATION: [], KEY_NAME_RES_PREPROCESSING: []}
        for item in items:
            if VALUE_TYPE_METHOD_NAME_AUGMENT in item.method_id:
                ...
                self.logger.warning(
                    f"Method id {item[MethodItem.FIELD_METHOD_ID]} with shorten method name not belong to {VALUE_TYPE_METHOD_NAME_AUGMENT}|{VALUE_TYPE_METHOD_NAME_PREPROCESS}"
                )
