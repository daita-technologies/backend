...
def CheckRunningAndDeleteTask(tableTask, identity_id, project_id, name):
    queryResp = tableTask.query(
        ...
        & (
            Attr("status").eq("PREPARING_DATA")
            | Attr("status").eq("PREPARING_HARDWARE")
    ...
    if len(queryResp["Items"]) > 0:
        raise Exception(
            f'There are {len(queryResp["Items"])} {name} tasks are running. Please stop all tasks before deleting the project!'
        )
    ...
    queryResp = tableTask.query(
        ...
        FilterExpression=Attr("project_id").eq(project_id)
        & (
            Attr("status").eq("CANCEL")
            | Attr("status").eq("FINISH")
            | Attr("status").eq("ERROR")
            | Attr("status").eq("FINISH_ERROR")
        ),
    )

    with tableTask.batch_writer() as batch:
        for each in queryResp["Items"]:
            tableTask.delete_item(
                Key={"identity_id": each["identity_id"], "task_id": each["task_id"]}
            )
...
def lambda_handler(event, context):
    try:
        ...
        try:
            CheckRunningAndDeleteTask(
                tableTask=tableGenerateTask,
                identity_id=identity_id,
                project_id=project_id,
                name="generate",
            )
            CheckRunningAndDeleteTask(
                tableTask=tableDataFlowstask,
                identity_id=identity_id,
                project_id=project_id,
                name="dataflow",
            )
            CheckRunningAndDeleteTask(
                tableTask=tableReferenceImages,
                identity_id=identity_id,
                project_id=project_id,
                name="reference image",
            )
            CheckRunningAndDeleteTask(
                tableTask=tableHealthycheckTask,
                identity_id=identity_id,
                project_id=project_id,
                name="healthycheck",
            )
        ...
        with tableHeathyCheckInfo.batch_writer() as batch:
            for each in queryTableHealthyCheckInfo["Items"]:
                tableHeathyCheckInfo.delete_item(
                    Key={
