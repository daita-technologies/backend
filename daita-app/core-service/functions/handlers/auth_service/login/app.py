...
def getUsernameByEmail(email):
    ...
    for _, data in enumerate(client['Users']):
        if data['UserStatus'] == 'EXTERNAL_PROVIDER':
            continue
        for info in data['Attributes']:
            if info['Name'] == 'email' and info['Value'] == email:
                if data['UserStatus'] == "CONFIRMED":
                    return data['Username'], None
                elif data['UserStatus'] == "UNCONFIRMED":
                    return data['Username'], MessageUserVerifyConfirmCode
    return None, None
...
class LoginClass(LambdaBaseClass):
    ...
    def handle(self, event, context):
        ...
        if re.fullmatch(mailRegexString, self.username):
            self.username, err = getUsernameByEmail(email=self.username)
        ...
        if not model.checkFirstLogin(ID=sub, username=self.username):
