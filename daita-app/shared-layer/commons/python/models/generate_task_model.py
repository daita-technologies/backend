...
class GenerateTaskItem():
    ...
    def from_db_item(self, item_info):
        if item_info is None:
            ...
            self.number_finished = int(
                item_info.get(self.FIELD_NUMBER_FINISHED))
            self.number_gen_images = int(
                item_info.get(self.FIELD_NUM_GEN_IMAGES))
            ...
            self.messages_in_flight = int(
                item_info.get(self.FIELD_MESSAGEINFLIGHT))
            return self
    ...
    def query_running_tasks(self, identity_id, project_id):
        response = self.table.query(
            KeyConditionExpression=Key(
                GenerateTaskItem.FIELD_IDENTITY_ID).eq(identity_id),
            ...
            Attr(GenerateTaskItem.FIELD_STATUS).ne(
                VALUE_GENERATE_TASK_STATUS_CANCEL)
        )
