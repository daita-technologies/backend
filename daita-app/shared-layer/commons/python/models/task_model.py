...
class TaskModel:
    ...
    def _paginate_query_w_option(
        ...
        ex_start_key=None,
        index_table=None,
        text_print="response from query",
    ):
    ...
    def _query_task(
        ...
        if pag_page_token is None:
            ...
            return_ls_items, do_continue = self._paginate_query_w_option(
                key_condition_exp,
                filter_exp,
                limit_size,
                projection_str,
                ...
                text_print="response from query pagetoken none FIRST",
            )

            while do_continue:
                temp = do_continue.copy()
                ls_items, do_continue = self._paginate_query_w_option(
                    key_condition_exp,
                    filter_exp,
                    limit_size,
                    projection_str,
                    attr_name,
                    attr_value,
                    ex_start_key=do_continue,
                    index_table=index_table,
                    text_print="response from query in WHILE LOOP",
                )
                if len(ls_items) > 0:
    ...
    def get_tasks_w(
        ...
        if filter_process_type in [
            VALUE_PROCESS_TYPE_AUGMENT,
            VALUE_PROCESS_TYPE_PREPROCESS,
        ]:
        ...
        ls_task, ls_page_token = self._query_task(
            pag_page_token,
            key_condition_exp,
            filterExpression,
            limit_size,
            projection_str=projecttion_str,
            attr_name=attr_name,
    ...
    def update_status(self, task_id, identity_id, status):
        response = self.table.update_item(
            Key={self.FIELD_TASK_ID: task_id, self.FIELD_IDENTITY_ID: identity_id},
    ...
    def update_attribute(self, task_id, identity_id, ls_update):
        ...
        """
        exprAttValue = {":up_ti": convert_current_date_to_iso8601()}
        exprAttName = {"#up_ti": self.FIELD_UPDATED_TIME}
    ...
    def create_task_reference_image(
        self,
        identity_id,
        project_id,
        project_name,
        ...
        self.table.put_item(Item=dict_info)

        return task_id, process_type

    def get_task_info_w_atribute(
        self,
        identity_id,
        task_id,
        ls_attribute=[FIELD_TASK_ID, FIELD_STATUS, FIELD_PROCESS_TYPE],
    ):
        a = {self.FIELD_IDENTITY_ID: identity_id, self.FIELD_TASK_ID: task_id}
        print("key structure: ", a)
        print("self.tablename: ", self.table_name)
        print("key_schema: ", self.table.key_schema)
        response = self.table.get_item(
            Key={self.FIELD_IDENTITY_ID: identity_id, self.FIELD_TASK_ID: task_id}
        )
