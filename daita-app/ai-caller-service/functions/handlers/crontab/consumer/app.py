...
def lambda_handler(event, context):
    ...
    while True:
        ...
        QueueResp = queueSQS.receive_messages(
            VisibilityTimeout=10, WaitTimeSeconds=0, MaxNumberOfMessages=10
        )
        pushedMessage = 0
        ...
        for index, message in enumerate(QueueResp):
            ...
            if (
                item.status == "FINISH"
                or item.status == "CANCEL"
                or item.status == "FINISH_ERROR"
            ...
            if item.status != "PENDING" or not item.waitingInQueue:
                ...
                item.status == "PENDING"
                and pushedMessage < maxPushedMessage
                and item.waitingInQueue
            ):
