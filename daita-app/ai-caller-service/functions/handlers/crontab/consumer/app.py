...
def lambda_handler(event, context):
    ...
    while True:
        ...
        queueSQS = sqsResourse.get_queue_by_name(
            QueueName=os.environ['TASK_QUEUE'])
        ...
        for index, message in enumerate(QueueResp):
            ...
            item = generate_task_model.get_task_info(
                detail['identity_id'], detail['task_id'])
            ...
            if item.status != 'PENDING' or not item.waitingInQueue:
                ...
                generate_task_model.update_task_dequeue(
                    detail['identity_id'], detail['task_id'])
                pushedMessage += 1
                print(response)
            ...
            if item.status == 'CANCEL' and int(item.number_finished) != 0:
                task_model.update_number_files(
                    task_id=item.task_id, identity_id=item.identity_id, num_finish=0)
                if item.process_type == 'AUGMENT':
                    tableDataGen = db_resource.Table(
                        os.environ['TABLE_DATA_AUGMENT'])
                else:
                    tableDataGen = db_resource.Table(
                        os.environ['TABLE_DATA_PREPROCESS'])
                deleted_file = []
                queryResponse = tableDataGen.query(
                    KeyConditionExpression=Key(
                        'project_id').eq(item.project_id),
                    FilterExpression=Attr('s3_key').contains(item.task_id), Limit=200
                )

                if 'Items' in queryResponse:
                    deleted_file = queryResponse['Items']
                else:
                    continue
                while 'LastEvaluatedKey' in queryResponse:
                    ...
                    queryResponse = tableDataGen.query(
                        KeyConditionExpression=Key(
                            'project_id').eq(item.project_id),
                        FilterExpression=Attr('s3_key').contains(item.task_id), Limit=200, ExclusiveStartKey=key
                    )
                    deleted_file.extend(queryResponse['Items'])
                print(f"The number files will be deleted {len(deleted_file)}")

                if len(deleted_file) > 0:
                    total_file = len(deleted_file)
                    total_size = 0
                    with tableDataGen.batch_writer() as batch:
                        for each in deleted_file:
