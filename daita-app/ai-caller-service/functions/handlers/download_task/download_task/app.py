...
class S3(object):
    ...
    def download_images(self, images, task_id):
        ...
        batch_size = int(constModel.get_num_value(
            code='limit_request_batchsize_ai', threshold='THRESHOLD'))
        batch_input = []
        batch_output = []
        total_len = 0
        ...
        for index,  batch in enumerate(batcher(generator(), batch_size)):
            ...
            self.s3.put_object(
                ...
                Bucket=self.bucket,
                Key=os.path.join(jsonInputLoads, str(
                    index)+'_download_image.json')
            )
            batch_input.append(self.bucket+'/'+jsonInputLoads +
                               '/'+str(index)+'_download_image.json')
        ...
        for index, it in enumerate(images):
            ...
            self.s3.put_object(
                ...
                Bucket=self.bucket,
                Key=os.path.join(self.folder, str(
                    index)+'_download_image.json')
            )
            download_images.append(
                {'path': self.bucket+'/'+self.folder+'/'+str(index)+'_download_image.json'})
        info = {
...
def downloadS3ToEFS(data):
    ...
    images = data['images']
    s3Resp, download_images = s3.download_images(
        images=images, task_id=data['task_id'])
    result = data
