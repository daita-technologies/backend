...
class ImageLoader(object):
    ...
    def __call__(self, info_image):
        ...
        if type_method == "PREPROCESS":
            ls_process = [
                item["s3_key"] for item in ls_data
            ]  # use all data in original for preprocessing
...
def lambda_handler(event, context):
    ...
    data["num_aug_per_imgs"] = (
        body["num_aug_per_imgs"] if "num_aug_per_imgs" in body else 1
    )
    ...
    return generate_response(message="OK", status_code=HTTPStatus.OK, data=data)
