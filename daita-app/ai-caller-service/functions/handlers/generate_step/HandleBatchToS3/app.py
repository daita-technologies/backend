...
def lambda_handler(event, context):
    ...
    if event["response"] == "OK":
        outputBatchDir = "/" + "/".join(
            event["batch"]["request_json"]["output_folder"].split("/")[2:]
        )
        ...
        generate_task_model.update_messages_in_flight(
            event["identity_id"], event["task_id"], len(infoUploadS3)
        )
    return {
