...
class RICalculateClass(LambdaBaseClass):
    ...
    def handle(self, event, context):
        ...
        (
            ...
            update_data,
            overlimit_image,
        ) = self.preprocessor.get_reference_image_paths(data, self.ls_method_id)
        save_dict = {"input_data": update_data, "reference": refer_dict}
        ...
        self.s3.put_object(
            Body=json.dumps(save_dict), Bucket=self.bucket, Key=s3_key_path
        )
        s3_name_over = f"output/RI_{self.index}_overlimit_image.json"
        s3_key_path_over = os.path.join(folder, s3_name_over)
        self.s3.put_object(
            Body=json.dumps(overlimit_image), Bucket=self.bucket, Key=s3_key_path_over
        )
