...
class GetDataClass(LambdaBaseClass):
    ...
    def handle(self, event, context):
        ...
        self.task_model.update_attribute(
            self.task_id,
            self.identity_id,
        ...
        if len(ls_info) > 0:
            bucket, folder = get_bucket_key_from_s3_uri(
                ls_info[0][DataItem.FIELD_S3_KEY]
            )
            folder = "/".join(folder.split("/")[:-1])
            s3_key_path = os.path.join(
                folder, f"reference_image_step_output/{self.task_id}/RI_getdata.json"
            )
            self.s3.put_object(
                Body=json.dumps(ls_batchs), Bucket=bucket, Key=s3_key_path
            )
        else:
            bucket = None
            s3_key_path = None
