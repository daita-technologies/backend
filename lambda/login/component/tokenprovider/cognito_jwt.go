...
func GetToken(accesToken string) (bool, TokenInformationResponse) {
	region := common.REGION
	userPoolID := common.USERPOOLID
	jwkURL := fmt.Sprintf("https://cognito-idp.%v.amazonaws.com/%v/.well-known/jwks.json", region, userPoolID)
	jwk := getJWK(jwkURL)
	token, err := validateToken(accesToken, region, userPoolID, jwk)
	isCheckValid := false
	if err != nil || !token.Valid {
		fmt.Printf("token is not valid\n%v", err)
		return isCheckValid, TokenInformationResponse{}
	} else {
		isCheckValid = true
	}
	return isCheckValid, TokenInformationResponse{
		Username: *MappingStringInfo("username", token.Claims.(jwt.MapClaims)),
		ClientID: *MappingStringInfo("client_id", token.Claims.(jwt.MapClaims)),
		EventId:  *MappingStringInfo("event_id", token.Claims.(jwt.MapClaims)),
	}

}
func GetInfoFromToken(accesToken string, field string) (string, error) {
	region := common.REGION
	userPoolID := common.USERPOOLID
	jwkURL := fmt.Sprintf("https://cognito-idp.%v.amazonaws.com/%v/.well-known/jwks.json", region, userPoolID)
	jwk := getJWK(jwkURL)
	...
	return *MappingStringInfo(field, token.Claims.(jwt.MapClaims)), nil
}

func MappingStringInfo(field string, claims jwt.MapClaims) *string {
	if tokenExp, ok := claims[field]; ok {
