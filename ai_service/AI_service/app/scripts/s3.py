...
class S3(object):
    def __init__(self,project_prefix,process_type):
        self.s3 = boto3.client('s3',aws_access_key_id=os.environ["ACCESSKEYID"]  ,aws_secret_access_key=os.environ["SECRET_KEY"])
        self.bucket = None
        self.s3_key =  None
    ...
    def download_images(self,images,list_name_aug,folder):
        ...
        list_image = list(map(lambda x : x.replace('/home/ec2-user',''), list(glob.glob(input_dir+'/**'))))
    ...
    def upload_image(self,project_prefix ,database_method,process_type,list_aug ,output_dir,total_file):
        info = []
        ...
        while True:
            ...
            cur_length_file = len(list(filter(is_json,img_list)))  if process_type == 'AUGMENT' else len(img_list)
            logDebug.debug("[DEBUG] cur_length_file {} total {} \n".format(cur_length_file,total_file))
        ...
        for it_img in img_list:            
            ...
            if not 'json' in it_img and process_type == 'AUGMENT':
                ...
                with open(file_json) as f:
                    ...
                    if 'augment_name' in data and data['augment_name'] in database_method:
            ...
            if not 'json' in it_img:
                info.append({'filename': s3_namefile,'size': os.path.getsize(it_img),'gen_id':str(aug_code)})
