...
class DynamoDBService:
    ...
    def create_bd(self, table_name, ls_key_schema, billmode = None, provisionthrough= None):
        ...
        for item in ls_key_schema:
            va_attribute.append({
                ...
                'AttributeType': item[2]
            })
            va_key_schema.append({
                'AttributeName': item[0],
                'KeyType': item[1]
            })
        try:
            ...
            if e.response['Error']['Code'] == 'ResourceInUseException' and 'Table already exists' in e.response['Error']['Message']:                
