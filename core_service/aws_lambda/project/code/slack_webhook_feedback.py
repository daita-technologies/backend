...
class Feedback(object):
    ...
    def CheckKeyIsExist(self, ID):
        response = self.db_client.Table(self.TBL).get_item(Key={"ID": ID})
        if "Item" in response:
            return True
        return False
...
def lambda_handler(event, context):
    ...
    if req.status_code != 200:
        return generate_response(
            message=MessageSendFeedbackFailed, data={}, headers=RESPONSE_HEADER
        )
    return generate_response(
        message=MessageSendFeedbackSuccessfully, data={}, headers=RESPONSE_HEADER
    )
