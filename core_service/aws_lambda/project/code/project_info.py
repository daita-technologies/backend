...
from utils import (
    ...
    aws_get_identity_id,
    dydb_get_project_id,
    dydb_get_project_full,
)


def get_running_task(
    table_name, db_resource, ls_tasks, identity_id, res_projectid, task_type=""
):
    ...
    item_tasks = table.query(
        ...
        FilterExpression=Attr("status").ne("FINISH")
        & Attr("status").ne("ERROR")
        & Attr("status").ne("CANCEL")
        & Attr("project_id").eq(res_projectid),
    )
...
def lambda_handler(event, context):
    ...
    try:
        ...
        for method, s3_path in reference_images.items():
            ...
            reference_info[method] = {"s3_path": s3_path, "filename": filename}
