...
def lambda_handler(event, context):
    ...
    try:
        ...
        if len(ls_filename) > MAX_NUMBER_ITEM_QUERY:
            raise Exception(f"The number of items is over {MAX_NUMBER_ITEM_QUERY}")
    ...
    try:
        ...
        response = table.get_item(Key={"project_id": project_id, "type": "ORIGINAL"})
        if response.get("Item"):
            current_num_data = response["Item"].get("count", 0)
        else:
            current_num_data = 0
        if (
            len(ls_batch_request) - len(ls_data) + current_num_data
            > MAX_NUM_IMAGES_IN_ORIGINAL
        ):
            raise (
                Exception(
    ...
    return convert_response(
        {"data": ls_data, "error": False, "success": True, "message": None}
    )
