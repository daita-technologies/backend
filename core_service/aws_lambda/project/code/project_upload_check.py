    ...
    try:
        ...
        while start_idx<len(ls_batch_request):
            ...
            for data in response['Responses'][os.environ["T_DATA_ORI"]]:
                ls_data.append({'filename': data['filename']['S'], 'size': data['size']['N']})
        ...
        if len(ls_batch_request)-len(ls_data)+current_num_data>MAX_NUM_IMAGES_IN_ORIGINAL:
            raise(Exception(f'The number of images in original of project is over the limitation {MAX_NUM_IMAGES_IN_ORIGINAL}!'))
