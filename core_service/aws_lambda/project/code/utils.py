...
def get_num_prj(identity_id):
    ...
    response = table.query(
        KeyConditionExpression=Key("identity_id").eq(identity_id), Select="COUNT"
    )
...
def aws_get_identity_id(id_token):
    ...
    try:
        identity_response = identity_client.get_id(
            IdentityPoolId=os.environ["IDENTITY_POOL_ID"], Logins={PROVIDER: id_token}
        )
...
def dydb_update_project_data_type_number(
    db_resource, identity_id, project_name, data_type, data_number, times_generated
):
    try:
        table = db_resource.Table(os.environ["T_PROJECT"])
        response = table.update_item(
            ...
            ExpressionAttributeNames={"#DA_TY": data_type},
...
def create_single_put_request(dict_value):
    dict_re = {"PutRequest": {"Item": {}}}
    for key, value in dict_value.items():
        dict_re["PutRequest"]["Item"][key] = {value[0]: value[1]}
    return dict_re
...
def create_secret_hash(clientSecret: str, user: str, clientPoolID: str) -> str:
    hash = hashlib.sha256()
