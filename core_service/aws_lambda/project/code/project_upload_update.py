...
def lambda_handler(event, context):
    try:
        ...
        if len(ls_object_info) > MAX_NUMBER_ITEM_PUT:
            raise Exception(f"The number of items is over {MAX_NUMBER_ITEM_PUT}")
        ...
        for object in ls_object_info:
            ...
            request = {
                ...
                "size": object["size"],  # size must be in Byte unit
                "is_ori": object["is_ori"],
    ...
    try:
        if is_ori and thumbnail_key is None:
            ...
            response = db_client.update_item(
                ...
                Key={"project_id": {"S": project_id}, "type": {"S": type_method}},
                ExpressionAttributeNames={
                    "#SI": "total_size",
                    "#COU": "count",
                    "#NE": "num_exist_data",
