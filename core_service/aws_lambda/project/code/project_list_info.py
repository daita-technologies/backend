...
def lambda_handler(event, context):
    ...
    try:
        ...
        for item_project in items_project["Items"]:
            ...
            item_project["is_sample"] = item_project.get("is_sample", False)
            item_project["gen_status"] = item_project.get("gen_status", "FINISH")
            item_project["description"] = item_project.get("project_info", "")
        ...
        for item_task in items_task["Items"]:
            if len(item_task.get("project_id", "")) > 0:
                group_project_id[item_task["project_id"]]["ls_task"].append(item_task)
    ...
    return convert_response(
        {"data": ls_items, "error": False, "success": True, "message": None}
    )
