...
from utils import (
    ...
    aws_get_identity_id,
    dydb_get_project,
    dydb_update_project_data_type_number,
    dydb_update_class_data,
)
import const
...
def lambda_handler(event, context):
    try:
        ...
        num_aug_per_imgs = min(
            MAX_NUMBER_GEN_PER_IMAGES, body.get("num_aug_p_img", 1)
        )  # default is 1
        data_number = body[
            "data_number"
        ...
        # check if not ec2 free, we will return exception
        table_ec2 = db_resource.Table(os.environ["T_INSTANCES"])
        response = table_ec2.scan(FilterExpression=Attr("assi_id").eq("free"))
        if len(response["Items"]) <= 0:
            raise Exception(
                "No resource available now for running task, please comeback after few minutes!"
            )
        # else:
        #     raise Exception("OK {}".format(len(response['Items'])))
    ...
    # get const value
    try:
        MAX_TIMES_PREPROCESS_IMAGES = const.get_const_db(
            const.MAX_TIMES_PREPROCESS_IMAGES
        )
    ...
    try:
        ...
        response = table.query(
            ...
            & Attr("status").ne("FINISH")
            & Attr("status").ne("ERROR"),
        )
        print("get task running:", response.get("Items", []))
        if len(response.get("Items", [])) > 0:
            raise Exception(
                "You have been running a task in this project, please wait!"
            )
    except Exception as e:
        print("Error: ", repr(e))
        return convert_response(
    ...
    try:
        ...
        if type_method == "AUGMENT":
            if times_generated >= MAX_TIMES_AUGMENT_IMAGES:
                raise Exception(
                    const.MES_REACH_LIMIT_AUGMENT.format(MAX_TIMES_AUGMENT_IMAGES)
                )
            ...
            if times_preprocess >= MAX_TIMES_PREPROCESS_IMAGES:
                raise Exception(
                    const.MES_REACH_LIMIT_PREPROCESS.format(MAX_TIMES_PREPROCESS_IMAGES)
                )
            ...
            response = table.update_item(
                ...
                ExpressionAttributeValues={":ts": 0, ":co": 0},
    ...
    # send request to url
    try:
        url = "http://3.136.208.218:443/v1/api/request_ai"
        request_body = {
            "identity_id": identity_id,
            "id_token": id_token,
            # "images": ls_process,
