...
def getUsernameByEmail(email):
    client = cog_provider_client.list_users(UserPoolId=USERPOOLID)
    for _, data in enumerate(client["Users"]):
        for info in data["Attributes"]:
            if (
                info["Name"] == "email"
                and info["Value"] == email
                and data["UserStatus"] == "CONFIRMED"
            ):
...
def getSub(access_token):
    resp = cog_provider_client.get_user(AccessToken=access_token)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(e)
        return generate_response(
            message=MessageLoginFailed, data={}, headers=RESPONSE_HEADER, error=True
        )
    ...
    return generate_response(
        message=MessageSignInSuccessfully, data=response, headers=RESPONSE_HEADER
    )
