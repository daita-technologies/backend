...
def lambda_handler(event, context):
    ...
    try:
        ...
        response = table.query(KeyConditions={"assi_id": identity_id})
        if response.get("Items"):
            if len(response["Items"]) > 0:
                raise Exception(
                    f"This id {identity_id} already assiged to an instance before!"
                )

        # check list ec2_id available
        response = table.query(KeyConditions={"assi_id": "free"})
