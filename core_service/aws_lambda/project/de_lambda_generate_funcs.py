...
def deploy_lambda_generate(general_info, lambda_service):
    ...
    lambda_uri, lambda_version = lambda_service.deploy_lambda_function(
        f"staging-generate-list-method",
        [ROOT + "generate_list_method.py", ROOT + "utils.py", ROOT + "const.py"],
        {
    ...
    add_lambda_info_to_list(
        ls_lambda_val, lambda_uri, lambda_version, "generate", "list_method"
    )

    # generate images with augmentation or preprocessing
    lambda_uri, lambda_version = lambda_service.deploy_lambda_function(
        f"staging-generate-images",
        [ROOT + "generate_images.py", ROOT + "utils.py", PACKAGES, ROOT + "const.py"],
        {
            "USER_POOL_ID": general_info["USER_POOL_ID"],
            "IDENTITY_POOL_ID": general_info["IDENTITY_POOL_ID"],
            "T_METHODS": general_info["T_METHODS"],
    ...
    add_lambda_info_to_list(
        ls_lambda_val, lambda_uri, lambda_version, "generate", "images"
    )

    # get current progress
    lambda_uri, lambda_version = lambda_service.deploy_lambda_function(
        f"staging-generate-task-progress",
        [ROOT + "generate_task_progress.py", ROOT + "utils.py", ROOT + "const.py"],
        {
            "USER_POOL_ID": general_info["USER_POOL_ID"],
            "IDENTITY_POOL_ID": general_info["IDENTITY_POOL_ID"],
            "T_TASKS": general_info["T_TASKS"],
    ...
    add_lambda_info_to_list(
        ls_lambda_val, lambda_uri, lambda_version, "generate", "task_progress"
    )
