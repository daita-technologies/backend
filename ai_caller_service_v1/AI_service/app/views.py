...
host = {"PREPROCESS": "http://{}:8000/ai", "AUGMENT": "http://{}:8000/ai"}
try:
...
def AI_service_request(request):
    ...
    res = manage_task.apply_async(
        queue="low",
        kwargs={
            "data": data,
            "task_s3_id": test["task_s3_id"],
            "task_id": res_dynamodb.task_id,
    ...
    return Response(
        ...
        data={"message": "", "data": {"task_id": res_dynamodb.task_id}, "error": True},
    )
...
def AI_service_check_healthy(request):
    ...
    if not "task_id" in data:
        return Response(
            status=status.HTTP_400_BAD_REQUEST,
            data={"message": "Miss task_id parameter", "data": {}, "error": True},
        )
    res = AsyncResult(data["task_id"])

    return Response(
        status=status.HTTP_200_OK,
        data={"message": "", "data": {"status": res.state}, "error": False},
    )
...
def download_s3_task(info_s3):
    ...
    try:
        info = s3_instance.download_images(
            info_s3["images"], info_s3["augment_code"], info_s3["task_id"]
        )
    except Exception as e:
        logError.debug(
            "[ERROR] Task ID download images S3: {} - Error: {}\n".format(
                info_s3["task_id"], e
            )
        )
        return info
...
def manage_update_task(data):
    ...
    task_s3 = download_s3_task.apply_async(
    ...
    num_gens = (
        ...
        else num_augments_per_image * num_images
    )
    try:
        dynamodb = TasksModel()
        dynamodb.create_item(
            identity_id, task_id, project_id, num_gens, process_type, "", ""
        )
    except Exception as e:
        logError.error(
            "[ERROR] Task ID: {} - Dynamodb Create Task: {}\n".format(task_id, e)
        )
        return

    return {
        "task_s3_id": task_s3.task_id,
        "num_gens": num_gens,
        "project_prefix": project_prefix,
...
def manage_task(data, task_s3_id, task_id, num_images):
    ...
    payload_getIP = json.dumps(
        {
            "identity_id": data["identity_id"],
            "task_id": task_id,
            "num_process_image": num_images,
        }
    )
    ...
    try:
        ...
        logDebug.debug(
            "[DEBUG] Response staging-balancer-asy-get-ip: {}".format(json_result)
        )
        logInfo.info("[INFO] staging-balancer-asy-get-ip: {}".format(json_result))
        body = json.loads(json_result["body"])
        logInfo.info(
            "[INFO] staging-balancer-asy-get-ip body: {} and type : {}".format(
                json_result["body"], type(json_result["body"])
            )
        )
        IP = body["data"]["ip"]
        ...
        dynamodb.update_process_error(
            task_id=task_id, identity_id=identity_id, IP="", EC2_ID=""
        )
    ...
    dynamodb.update_process(
        task_id=task_id, identity_id=identity_id, num_finish=0, status="PREPARING_DATA"
    )
    ...
    if not bool(info):
        ...
        dynamodb.update_process_error(
            task_id=task_id, identity_id=identity_id, IP=IP, EC2_ID=EC2_ID
        )
        return {"message": "error"}
    ...
    if bool(EC2_ID):
        ...
        logInfo.info(
            "[INFO] insert_KV_EC2: Task id {} -  ec2 {}\n".format(task_id, EC2_ID)
        )
    ...
    def AI_service():
        ...
        logInfo.debug(
            "[INFO] TASK ID {} , request host {}".format(
                task_id, host[process_type].format(IP)
            )
        )
        info = {
    ...
    def check_dir():
        ...
        total = (
            length_image * num_augments_per_image
    ...
    while True:
        ...
        if which is quit:
            try:
                ...
                if bool(EC2_ID):
                    logInfo.info(
                        "[INFO] done_task_ec2: Task id {} -  ec2 {}\n".format(
                            task_id, EC2_ID
                        )
                    )
        ...
        # receive the message from thread request to update Tasks table
            ...
            try:
                ...
                logError.error(
                    "[ERROR] Task ID: {} - Update Process Task: {}\n".format(task_id, e)
                )
                return {"Error": e}
        # get error from error_message_queue
            ...
            if message == "Error_retry":
                count_retries += 1
            logInfo.info(
                "[INFO] error message Error_retry : {} batch_output : {}".format(
                    count_retries, len(batch_output)
                )
            )
            ...
            logInfo.info(
                "[INFO] Task ID {} ---- Request AI :{}".format(task_id, message)
            )
