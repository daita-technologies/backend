...
def request_finish_task_ec2(ec2_id,task_id):
    ...
    try :
        ...
        logDebug.debug("[DEBUG]staging-balancer-asy-finish-task: {}".format(json.loads(response['Payload'].read())))
...
def request_update_proj(update_pro_info,list_file_s3):
    ...
    for batch_file in batch_list_s3:
        ...
        try :
            update_project_output = requests.post('https://4cujdvfmd4.execute-api.us-east-2.amazonaws.com/staging/projects/upload_update',json=info)
        ...
        logDebug.debug("[DEBUG] Request Update Upload: {}\n".format(update_project_output))
...
class ThreadRequest(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            self.worker['log_request_AI'].put("Host {} input_json : {}".format(self.worker['host'],input_json))
            self.worker['log_request_AI'].task_done()
            
            request_task = request_AI_task.apply_async(queue='request_ai',kwargs={'host':self.worker['host'],'input_json':input_json})
...
def uploading_image(input_uploading_image):
    s3_instance = S3(input_uploading_image['project_prefix'],input_uploading_image['process_type'])
    s3_key,  s3_info = s3_instance.upload_image(input_uploading_image['project_prefix'],
...
@shared_task(bind=True,retries=1,autoretry_for=(Exception,),exponential_backoff=2,retry_backoff=True,retry_jitter=False,default_retry_delay=1,max_retries=15)
def request_AI_task(self,host,input_json):
    logInfo.info("[INFO] The time Retry {} max retries : {} \n".format(self.request.retries,self.max_retries))
    if self.request.retries >= self.max_retries:
        logError.error("[ERROR] MAX RETRIES Request Max Retry : {} , Retry times : {} \n".format(self.max_retries,self.request.retries))
    ...
    total_image = input_json['num_augments_per_image']*len(input_json['images_paths']) if 'num_augments_per_image' in input_json else len(input_json['images_paths'])
